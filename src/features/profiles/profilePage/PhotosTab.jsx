...
import { deleteFormFirebaseStoreage } from "../../../app/firestore/firebaseService";
import {
  deletePhotoFormCollection,
  getUserPhotos,
  setMainPhoto,
...
export default function PhotosTab({ profile, isCurrentUser }) {
  ...
  async function handleDeletePhoto(photo, target) {
    ...
    try {
      await deleteFormFirebaseStoreage(photo.name);
      await deletePhotoFormCollection(photo.id);
    } catch (error) {
      toast.error(error.message);
    } finally {
        ...
        <Grid.Column width={16}>
          ...
          {isCurrentUser && (
            <Button
              ...
              basic
              content={editMode ? "Cancle" : "Add Photo"}
            />
          )}
        </Grid.Column>
        <Grid.Column width={16}>
